import type { TiledMap, TiledObject, TiledObjectLayer, TiledTileLayer, TiledTileset } from "@/types/Tiled";
import { hexToRgb } from "@/utils/colors";
import sharedFetch from "@/utils/sharedFetch";
import type { ImageObj } from "./ImageLoader";
import ImageLoader from "./ImageLoader";
// @ts-ignore
import noise from "@/vendor/noise";
...
export default class MapLoader {
  ...
  private tilesetPromises: { [key: string]: Promise<IsometricTileset> } = {};
  private tilesets: { [key: string]: IsometricTileset } = {};
  ...
  private async loadMapData(
    ...
    const map = await this.parseMap(mapJson, tileLayer, customTiles, rootPath, name);
  ...
  async parseMap(
    ...
    if (mapJson.orientation === "isometric") {
      // do something...
      const GameTile = await this.parseWall(mapJson, urlFolder);
  ...
  async parseTriggers(json: TiledMap, rootPath: string, map: MapData): Promise<GameTrigger[]> {
    ...
    if (triggers) {
      triggers.objects.forEach((object) => {
        if (json.orientation === "isometric") {
          const mapPosition = {
            x: (object.x - object.width / 2) / 640,
            y: (object.y - object.height / 2) / 640,
          };
          object.x = mapPosition.x;
          object.y = mapPosition.y;
        } else {
          const mapPosition = {
            x: (object.x + object.width / 2) / json.tilewidth,
            y: (object.y + object.height / 2) / json.tileheight,
          };
          object.x = mapPosition.x;
          object.y = mapPosition.y;
        }
  ...
  public async loadSkin(name: string, url: string): Promise<void> {
    ...
    const floors = await this.parseFloor(mapJson.name, mapJson.floor, urlFolder);
  ...
  public async loadMapTile(name: string, url: string): Promise<void> {
    const response = await sharedFetch(url);
    const mapJson = (await response.json()) as TiledMap;
    const urlFolder = url.substring(0, url.lastIndexOf("/"));

    const GameTile = await this.parseWall(mapJson, urlFolder);
    this.mapTiles[name] = GameTile;
  }

  async parseWall(json: TiledMap, urlPath: string): Promise<GameTile> {
    const wall = {
      tilesets: [],
      layers: [],
      imageData: null,
    ...
    wall.layers.forEach((layer) => {
      if (layer.type !== "objectgroup") this.parseSprites(json, urlPath, layer, wall.tilesets, wall.sprites);
    });
  ...
  getTiledImageFromGid(gid: number, urlPath: string, map: TiledMap) {
    ...
    const source = map.tilesets.find((tileset, index) => {
      const nextFirstGuid = map.tilesets[index + 1]?.firstgid ?? Infinity;
      if (tile < nextFirstGuid) {
    ...
    const path = url.resolve(urlPath, source) as string;
    ...
    const tileImage = tileset.images[tile];
  ...
  async loadTileset(tileset: TiledTileset, urlPath: string): Promise<IsometricTileset> {
  ...
  async parseTileset(tileset: TiledTileset, urlPath: string): Promise<IsometricTileset> {
    const isometricTileset = {
    ...
    const sourceDirectory = tileset.source.split("/").slice(0, -1).join("/") + "/";
    const path = url.resolve(urlPath, tileset.source) as string;

    const source = await sharedFetch(path);
    const json = (await source.json()) as any;
    if (json.tiles) {
      isometricTileset.type = "imagegroup";
      await Promise.all(
        json.tiles.map(async (tile: any) => {
          ...
          isometricTileset.images[tile.id] = tileImage;
        })
      );
    } else {
      ...
      isometricTileset.type = "tileset";
      const imagePath = `${urlPath}${sourceDirectory}${imageSrc}`;
      const name = `${sourceDirectory}${imageSrc}`.replaceAll("/", "__").split(".").shift() as string;
      const image = await ImageLoader.getInstance().loadImage(name, imagePath, { skipPixi: false });
      const tileImage = {
        image,
        name,
      ...
      isometricTileset.images[0] = tileImage;
    }

    this.tilesets[path] = isometricTileset;

    return isometricTileset;
  }
  ...
  async parseFloor(
    name: string,
    { stamp, density, stampColor }: { stamp: string; density: number; stampColor: string },
    urlFolder: string
